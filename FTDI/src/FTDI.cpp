...
FTDI::FTDI (int vid, int pid, int freq, int interface) {
    ...
    if(ftdi_init(&mFtdi) != 0) {
        throw std::runtime_error("Error: FTDI not initialised");
    }
...
void FTDI::write(uint8_t addr, const std::vector<uint8_t>& write_data) {
...
void FTDI::send(uint8_t addr, const std::vector<uint8_t>& write_data, std::vector<uint8_t>& read_data) {
